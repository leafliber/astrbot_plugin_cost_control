...
超出 ``budgets`` 配置的阈值。超限维度与 ``over_limit_policy`` 一起返回，
供 ``on_llm_request`` 决定拦截（``stop_llm``）或切换备用 provider。
...
class BudgetMixin:
    ...
    def get_over_limit_policy(self) -> dict[str, Any]:
        """返回超限处理策略（``over_limit_policy`` 配置）。"""
        return get_config(getattr(self, "config", None), "over_limit_policy", {}) or {}
    ...
    async def check_budget(self, umo: str, model: str | None) -> dict[str, Any]:
        ...
        Returns:
            ``{"exceeded": bool, "dim": str | None, "limit": int, "used": int,
            "policy": dict}``。任何异常都降级为「未超限」，绝不阻断主流程。
        """
